...
def count_misses(screen, misses, Gunshot, explosion, gameover):
    '''End game; closes window after playing sound effects and displaying graphics.'''
    if misses.count == 3:
        x = 0
        screen.blit(gameover.image, gameover.rect)
        while x < 5:
            pygame.display.flip() # *Show last miss to 3.
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    sys.exit()
            gunshots(screen, Gunshot)
            x += 1
        pygame.time.wait(3000)
        sys.exit()

def gunshots(screen, Gunshot):
    pygame.mixer.music.load(r'C:\Users\Alvin\Desktop\desktop\python_work\Projects\alien_invasion\13.6.py\sound effects\gunshot.mp3')
    gunshot = Gunshot(screen)
    screen.blit(gunshot.image, gunshot.rect)
