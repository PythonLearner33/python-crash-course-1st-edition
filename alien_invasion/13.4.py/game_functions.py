...
def create_raindrop(ai_settings, screen, raindrops, raindrop_number, row_number):
    ...
    raindrop_number = randint(0, 13)
    raindrop.x = raindrop_width + 2 * raindrop_width * raindrop_number
    raindrop.rect.x = raindrop.x
    raindrop.y = -1200 + 2 * raindrop.rect.height * row_number
...
def check_length_raindrops(ai_settings, screen, raindrops):
    if len(raindrops) == 0:
        time.sleep(1)
...
def update_raindrops(ai_settings, screen, raindrops):
    ...
    check_length_raindrops(ai_settings, screen, raindrops)
    raindrops.update()